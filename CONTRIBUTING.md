...
This document defines the protocols for setting up the development environment, understanding the underlying technology stack, and compiling **恩纳 (Enna)** from the source.
...
The 恩纳 (Enna) codebase is built on a modern, reactive architecture. Contributors are expected to have a high level of familiarity with:
...
恩纳 (Enna) follows a strict **Clean Architecture** approach. This separation of concerns ensures that the audio engine remains independent of the UI layer.
...
1. **Clone the Source:**
```bash
git clone https://github.com/cgens67/enna.git
cd 恩纳 (Enna)
...
2. **Secret Management:**
恩纳 (Enna) uses a modular properties system. If your build requires specific API keys (e.g., Discord Client IDs), define them in your `local.properties`:
```properties
...
<div align="center">
<sub>恩纳 (Enna): Engineering audio freedom. Part of the <strong>恩纳 (Enna)</strong> ecosystem.</sub>
</div>